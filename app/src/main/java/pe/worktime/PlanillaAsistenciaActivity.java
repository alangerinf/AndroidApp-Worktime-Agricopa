...
public class PlanillaAsistenciaActivity extends AbstractActivity {
	...
	private static PlanillaAsistenciaActivity actual;
			...
			public void onItemClick(AdapterView<?> arg0, View arg1,
				...
				try{
					...
					if (ContextTest.idxTipoEntradaIngreso >= 0) {
						Application.context.getHorasConsumidorController().isValidSalida();
					...
					if (ContextTest.idxTipoEntradaSalida >= 0) {
						...
						Application.context.getHorasConsumidorController().isValidSalida();
						Intent ites = new Intent(getApplicationContext(),SalidaTabActivity.class);
						startActivity(ites);
					}
					
				
					if (ContextTest.idxTipoEntradaTransferencia == 0 || ContextTest.idxTipoEntradaTransferencia == 1 ) {
						Application.context.getHorasConsumidorController().isValidTransferencia();
...
//						List<HorasConsumidorDetalle> items = Application.context.getHorasConsumidorController().getSelected().getDetalle();
//						for (HorasConsumidorDetalle item : items) {
//							if (!item.getCodTrabajador().equalsIgnoreCase("")){
//								Application.context.getProductividadController().addDetalle(item.getCodTrabajador(),0);
//							}
//						}
...
//					}
	...
	public void onBackPressed (){
		...
		if(ContextTest.idxTipoEntradaTransferencia == 3) {
			Intent itet = new Intent(getApplicationContext(),TipoEntradaTransferenciaActivity.class);
			startActivity(itet);
		}else{
			super.onBackPressed();
		}
